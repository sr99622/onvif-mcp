...
Complete `GPG_KEY_BACKUP.md` before starting this runbook. Its key export must
already be verified on the backup share.
...
1. **The GPG key is created and backed up first**, by the user in a real terminal
   following `GPG_KEY_BACKUP.md`. Its passphrase must never pass through an agent
   session, command line, or file. This runbook only verifies the key and uses it
   to initialize `pass`.
...
## 2. Verify the backed-up GPG key and initialize the store

The user has already completed `GPG_KEY_BACKUP.md`. Verify the key and its
backup before storing CA passphrases:
...
Initialize the store with that fingerprint. Bare `pass init` fails with a usage
error on some boxes; an explicit recipient also avoids selecting the wrong key:

```bash
pass init YOUR_FULL_FINGERPRINT
cat ~/.password-store/.gpg-id      # must equal YOUR_FULL_FINGERPRINT
```
...
sha256sum \
  "{{CA_ROOT_PATH}}/backups/password-store-backup-{{DATE}}.tar.gz" \
  "{{BACKUP_PATH}}/Camera-CA-Backups/password-store-backup-{{DATE}}.tar.gz"   # hashes must match exactly
```
...
`GPG_KEY_BACKUP.md`. Confirm its file is still present at
...
- **Create and back up the GPG key first.** Follow `GPG_KEY_BACKUP.md` in the
  user's terminal before this runbook. Agent-side creation with a placeholder
  passphrase once left the user facing a prompt for a secret nobody knew.
