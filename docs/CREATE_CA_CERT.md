...
Complete `GPG_KEY.md` before starting this runbook. Its key export must
...
1. **The GPG key is created and backed up first**, by the user in a real terminal
   following `GPG_KEY.md`. Its passphrase must never pass through an agent
...
The user has already completed `GPG_KEY.md`. Verify the key and its
...
- **Create and back up the GPG key first.** Follow `GPG_KEY.md` in the
